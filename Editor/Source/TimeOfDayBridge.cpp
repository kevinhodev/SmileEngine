...
namespace SmileEditor {
    ...
    void TimeOfDayBridge::SetRenderer(RendererHandle _Renderer) {
        Renderer = _Renderer;
    ...
    // ---- Acesso ao estado ----
    class LockedTimeOfDay {
    public:
        explicit LockedTimeOfDay(const RendererHandle& _Renderer)
            : Access(_Renderer ? _Renderer.Lock() : RendererHandle::Access{}) {
            Value = Access ? &Access->GetTimeOfDay() : &FallbackTod();
        }

        Smile::FTimeOfDay* operator->() const { return Value; }

    private:
        RendererHandle::Access Access;
        Smile::FTimeOfDay*     Value = nullptr;
    };

    static LockedTimeOfDay TodOf(const RendererHandle& _R) { return LockedTimeOfDay(_R); }
    static LockedTimeOfDay TodOfConst(const RendererHandle& _R) { return LockedTimeOfDay(_R); }
