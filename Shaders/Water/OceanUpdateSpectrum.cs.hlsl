...
// Evolucao temporal do espectro (porte de updatespectrum.comp do Asylum).
// H0Tex e (N+1)^2 = (h0(k).x, h0(k).y, omega(k), 0); o vento ja esta bakado no h0
// (FOceanFFT computa H0 na CPU com a direcao do vento atual).
// Saidas: SpecH = h(k,t) e SpecD = D(k,t) (Dx + i*Dz, truque de linearidade do DFT).
...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    float4 hk    = H0Tex.Load(int3(loc1, 0)); // (h0(k).xy, omega, 0)
    float2 h0_k  = hk.xy;
    float  w_k   = hk.z;
    ...
    // D(k,t) = -i (k/|k|) h(k,t). Direcao de k (escala irrelevante -> normalize).
