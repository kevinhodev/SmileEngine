...
namespace Smile {
    void FDescriptorHeap::Initialize(ID3D12Device* _Device,
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE FDescriptorHeap::CpuHandle(UINT _Index) const {
        D3D12_CPU_DESCRIPTOR_HANDLE Handle = DescriptorHeap->GetCPUDescriptorHandleForHeapStart();
        Handle.ptr += static_cast<SIZE_T>(_Index) * HandleSize;
        return Handle;
    }

    D3D12_GPU_DESCRIPTOR_HANDLE FDescriptorHeap::GpuHandle(UINT _Index) const {
        D3D12_GPU_DESCRIPTOR_HANDLE Handle = DescriptorHeap->GetGPUDescriptorHandleForHeapStart();
        Handle.ptr += static_cast<UINT64>(_Index) * HandleSize;
        return Handle;
    }
} 
